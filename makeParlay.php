...
$status = "pending";
$credit = htmlentities($json_obj['available_credit']);
...
$stmt = $mysqli->prepare("insert into wagers (description, event, risk, win, username, status) values (?,?,?,'300','123',?)");
...
$stmt->bind_param('ssis', $bet_description,$bet_event,$bet_amount, $status);
$stmt->execute();
$stmt->close();
